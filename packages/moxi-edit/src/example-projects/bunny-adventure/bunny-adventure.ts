...
export const init = (async () => {
  ...
  const backgroundContainer = createTileGrid({
    width: 64,              // 64 tiles wide
    height: 64,             // 64 tiles high
    cellWidth: 16,          // 16px per tile
    cellHeight: 16,         // 16px per tile
    centered: true          // Center the grid
  }, tileVariations);
  ...
  movementBehavior.init(bunny, scene.renderer, characterFrames);
