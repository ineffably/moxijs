...
export class UICheckbox extends UIComponent {
  ...
  public setChecked(checked: boolean): void {
    ...
    // Parent should update the prop via updateChecked()
  ...
  private updateVisuals(): void {
    ...
    // Disabled state colors (use theme resolver if available)
    const disabledBgColor = this.themeResolver?.getColor('disabled') ?? 0x666666;
    const disabledBorderColor = this.themeResolver?.getColor('border') ?? 0x444444;
    const disabledCheckedBgColor = this.themeResolver?.getColor('disabled') ?? 0x555555;

    // Draw checkbox background
    if (this.checked) {
      // Checked: filled background
      const bgColor = this.props.disabled ? disabledCheckedBgColor : this.props.checkedBackgroundColor;
      const borderColor = this.props.disabled ? disabledBorderColor : this.props.checkedBackgroundColor;
      ...
      const bgColor = this.props.disabled ? disabledBgColor : this.props.backgroundColor;
      const borderColor = this.props.disabled 
        ? disabledBorderColor 
        : (isHovered ? 0x999999 : this.props.borderColor);
      this.checkboxGraphics.roundRect(0, 0, size, size, borderRadius);
      this.checkboxGraphics.fill({ color: bgColor });
      this.checkboxGraphics.stroke({ color: borderColor, width: 1 });
    }
    ...
    if (this.checked) {
      ...
      // Use muted checkmark color when disabled (use theme resolver if available)
      const disabledCheckmarkColor = this.themeResolver?.getColor('disabled') ?? 0x888888;
      const checkmarkColor = this.props.disabled ? disabledCheckmarkColor : this.props.checkColor;
      ...
      this.checkmarkGraphics.stroke({ color: this.props.checkColor, width: 2 });
    }
  }
  ...
  public updateChecked(checked: boolean): void {
    if (this.isControlled && checked !== this.checked) {
      this.checked = checked;
      this.updateVisuals();
    }
  }
